...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        enc_hiddens, dec_init_state = self.encode(source_padded, source_lengths) 
        # 返回encoder的隐含层以及decoder的init c和h
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # source-padded is a tensor with indices
        # source 是embedding，输入索引（句子里的词已经转成id了），返回对应的embedding
        X = self.model_embeddings_source(source_padded) # (20.5) -> (20,5,3)

        # size (20,5,3), contain 5 sentences # source_lengths [20, 12, 9, 8, 5]
        # after packed, padding is removed and sentences are stacked together
        X = pack_padded_sequence(X, source_lengths) # size (54, 3) make sure lstm won't process the padding data
        # 不太理解batch_sizes怎么回事
        # !!!!!!是竖着执行的，只有5个5是因为最短的是5个，太牛了

        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        print(torch.cat((last_hidden[0], last_hidden[1]),dim=1).shape)
        # hidden size: 3，但因为双向，所以是6
        # last_hidden, cell: 一样的维度，都是2*5*3，双向*每个句子的最后一个h*h层维度
        
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0] # get data padded again
        # 变成 20*5*6，双向*3

        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=1)) #变成5，6
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=1))
        dec_init_state = (init_decoder_hidden, init_decoder_cell) # make a tuple
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) # 5，20，6 -> 5,20,3
        Y = self.model_embeddings_target(target_padded)
        for Y_t in torch.split(Y, 1): # 分割多维矩阵 20,5,3 -> 1,5,3，从词开始
            Y_t = torch.squeeze(Y_t, dim=0) # 1,5,3 -> 5,3
            Ybar_t = torch.cat((Y_t, o_prev), dim=1) # 5,6
            # o_prev一开始是0
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        # 先把单个词送进decoder中 Ybar_t(5,6)
        dec_state = self.decoder(Ybar_t, dec_state)

        dec_hidden, dec_cell = dec_state # (5,3)，单向，隐含层维度为3

        # enc_hiddens_proj: (5,20,3)
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2)) # (b,src_len,1)
        e_t = torch.squeeze(e_t, dim=2)
        # e_t: (5,20), 对长度20，每一个都有一个分数（softmax前），ba
