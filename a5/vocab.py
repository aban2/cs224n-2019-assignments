...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        char_padded_sents = self.words2charindices(sents)
        padded_sents = pad_sents_char(sents, char2id['<pad>'])
        tensor_sents = torch.tensor(padded_sents, dtype=torch.long, device=device)
        tensor_sents = torch.t(tensor_sents)
